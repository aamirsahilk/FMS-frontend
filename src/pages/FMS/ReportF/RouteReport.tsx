...
interface Route {
  ...
  destination_Country: { country_name: string };
  origin_Country: { country_name: string };
  origin_State: { state_name: string };
  destination_State: { state_name: string };
  destination_City: { city_name: string };
  origin_City: { city_name: string };
  borders: [];
}
...
const Routereport = () => {
  ...
  const [routesReports, setRoutesReports] = useState<RouteReport[]>([]);
  ...
  useEffect(() => {
    dispatch(setPageTitle("Route Report"));
  }, []);

  const findBookingAmounts = (bookingId: any, type: string) => {
    const selectedBooking = routesReports.find(
  ...
  // Get Route Wise Reports
  const getRouteReport = async (route_id?: number | string) => {
    setIsLoading(true);
    ...
    try {
      ...
      if (response?.status === 200 && response?.data?.data) {
        setRoutesReports(response?.data?.data);
      }
    } catch (error: any) {
  ...
  const getAllRoute = async () => {
    try {
      const response = await axios.get(`${config.API_BASE_URL}/routes`);
  ...
  // CSV Data
  const handleCsvData = () => {
    if (routesReports && routesReports?.length) {
      const CSVData = routesReports?.map(
        (
          {
            border_Route,
            booking_id,
  ...
  const getFileName = (): string => {
    if (selectedRoute) {
      const routeName =
        routes?.find(({ route_id }) => route_id === Number(selectedRoute))
          ?.route_name || "";
      return `${routeName}-route-report`;
    } else {
      return "route-report-all";
    }
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between flex-wrap gap-4">
        <h2 className="text-xl">Route Report</h2>
        ...
        <div className="mt-5 panel p-0 border-0 overflow-hidden">
          <div className="table-responsive">
            <table className="table-striped table-hover">
              ...
              <tbody>
                {routesReports &&
                  routesReports?.length > 0 &&
                  routesReports.map((data, index) => {
                    return (
                      <tr key={index}>
                        <td>{index + 1}</td>
            ...
            {routesReports && !routesReports?.length && (
      ...
      {value === "GRID" && (
        <div className="grid 2xl:grid-cols-4 xl:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-6 mt-5 w-full">
          {routesReports &&
            routesReports?.length &&
            routesReports.map((contact: any) => {
              return (
                <div
                    ...
                    <form>
                      <div className="mt-4">
                        <div>
                          ...
                          >
                            ...
                            {routes &&
                              routes?.length > 0 &&
                              routes?.map(({ route_id, route_name }) => (
                      ...
                      <div className="flex justify-end items-center mt-8">
                        <button
                          ...
                          onClick={() => getRouteReport(selectedRoute)}
                          disabled={isLoading}
                        >
                          {isLoading ? (
