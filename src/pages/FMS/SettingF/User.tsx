...
const User = () => {
    ...
    useEffect(() => {
        ...
        const fetch = async () => {
            const { data } = await axios.get(`${config.API_BASE_URL}/users`);
            console.log(data, '>>>>>>>>>>>>>>>>>>');
            setUserData(data.data);
            console.log(userData);
            const  role  = await axios.get(`${config.API_BASE_URL}/userRole/userroles`);
    ...
    useEffect(() => {
        setFilteredItems(() => {
            return userData.filter((item: any) => {
                return item.username.toLowerCase().includes(search.toLowerCase()) || item.role.toLowerCase().includes(search.toLowerCase());
            });
        });
    ...
    const saveUser = async () => {
        ...
        if (params.user_id) {
            ...
            let addUSer = await axios.post(`${config.API_BASE_URL}/users`, params);
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            try {
                const  data  = await axios.post(`${config.API_BASE_URL}/users`,values);
                            ...
                            >
                                <Dialog.Panel className="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg text-black dark:text-white-dark">
                                    ...
                                    <div className="text-lg font-medium bg-[#fbfbfb] dark:bg-[#121c2c] ltr:pl-5 rtl:pr-5 py-3 ltr:pr-[50px] rtl:pl-[50px]">
                                        {params.client ? 'Edit Contact' : 'Add User'}
                                    </div>
                                    <div className="p-5">
                                        <form onSubmit={formik.handleSubmit}>
                                            ...
                                            <div className="grid mt-4 grid-cols-1 sm:grid-cols-2 gap-4">
                                                ...
                                                <div>
                                                    <label htmlFor="country">Select Role</label>
                                                    <select id="ctnSelect1" name="role"
                                                    onChange={formik.handleChange}
                                                    onBlur={formik.handleBlur}
                                                    value={formik.values.role} className="form-select text-white-dark"   required>
