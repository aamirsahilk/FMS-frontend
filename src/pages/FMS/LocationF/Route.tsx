...
const Route = () => {
    ...
    useEffect(() => {
        const updatedSelectedBorderData : any = selectedValues?.map((selectedId) => {
            ...
            return selectedBorder ? _.pick(selectedBorder, ['country_id', 'borderName', 'type', 'charges', 'border_id']) : null;
        });
    ...
    const editUser = async (user: any = null) => {
        ...
        if (user) {
            ...
            const selectedBorderIds = user?.border?.map((border: any) => border.border_id);
    ...
    useEffect(() => {
        ...
        if (formik.values.originState) {
            ...
            const matchingState : any = filterState?.find((country : any) => country.states.some((state : any) => state.state_id === selectedStateId));
            console.log(matchingState, 'filterState');

            // Extract cities from the matching state or use an empty array
            const cities : any = matchingState?.states.find((state : any) => state.state_id === selectedStateId)?.citys || [];
    ...
    useEffect(() => {
        ...
        if (formik.values.destinationState) {
            const selectedStateId  : any = parseInt(formik.values.destinationState);

            // Find the matching state
            const matchingState : any = filterdestinationState?.find((country : any) => country.states.some((state : any) => state.state_id === selectedStateId));
            console.log(matchingState, 'filterState');
