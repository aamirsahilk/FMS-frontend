...
const Border = () => {
    ...
    const fetchBorderData = async () => {
        try {
            const { data } = await axios.get(`${config.API_BASE_URL}/routes/border/getAll`);
            return data;
        } catch (error) {
            console.error('Error fetching country data:', error);
            throw error;
    ...
    const deleteUser = async (border: any = null) => {
        try {
            await axios.delete(`${config.API_BASE_URL}/routes/border/${border.border_id}`);
            showMessage('Border has been deleted successfully.');
            await fetch();
            // const updatedBorderData = await fetchBorderData();
            // setBorderData(updatedBorderData);
        } catch (error) {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                if (params.border_id) {
                    const response = await axios.put(`${config.API_BASE_URL}/routes/border/${params.border_id}`, values);
                    ...
                    let Data = await axios.post(`${config.API_BASE_URL}/routes/border`, values);
                    if (Data.status === 201) {
                        showMessage(`Border created`);
                        formik.resetForm();
                        setAddContactModal(false);
                    }
                    formik.resetForm();
    ...
    const handleCloseModal = () => {
        setAddContactModal(false);
        formik.resetForm();
        // Fetch updated data after closing the modal
        fetchBorderData().then((updatedData) => setBorderData(updatedData));
    };
                            ...
                            >
                                <Dialog.Panel className="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg text-black dark:text-white-dark">
                                    ...
                                    <div className="text-lg font-medium bg-[#fbfbfb] dark:bg-[#121c2c] ltr:pl-5 rtl:pr-5 py-3 ltr:pr-[50px] rtl:pl-[50px]">
                                        {viewMode ? 'Border Details' : params.borderName ? 'Edit Border' : 'Add Border'}
                                    </div>
                                    <div className="p-5">
                                        <form onSubmit={formik.handleSubmit}>
                                            <div>
                                                <label htmlFor="borderName">Border Name</label>
                                                <input
                                                    id="borderName"
                                                    type="text"
                                                    name="borderName"
                                            ...
                                            <div className="mt-4">
                                                <label htmlFor="ctnSelect">Border type</label>
                                                <select
                                                    id="ctnSelect"
                                                    onChange={formik.handleChange}
                                                    onBlur={formik.handleBlur}
                                            ...
                                            {!viewContactModal && (
                                                <div className="flex justify-end items-center mt-8">
                                                    ...
                                                    <button type="submit" className="btn btn-primary ltr:ml-4 rtl:mr-4">
                                                        {params.border_id ? 'Update' : 'Submit'}
                                                    </button>
                                                </div>
                                            )}
