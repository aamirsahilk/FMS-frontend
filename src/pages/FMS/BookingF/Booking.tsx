...
const Booking = () => {
  ...
  const [viewDriverMode, setViewDriverMode] = useState<any>(false);
  ...
  const deleteUser = async (user: any = null) => {
    ...
    await axios.put(
      `${config.API_BASE_URL}/bookings/bookingstatus/${user.booking_id}`,
      data
    );
  ...
  const viewInvoice = async (user: any) => {
    let json1 = JSON.parse(JSON.stringify(user));
    const userNewBookingId = user?.new_booking_id;
    const matchingInvoice: any = invoiceDataAll.find(
      (invoice: any) => invoice?.booking?.new_booking_id === userNewBookingId
    );

    if (matchingInvoice) {
      const newBookingId: any = matchingInvoice?.booking?.new_booking_id;

      // Extract properties from the matching invoice
      const { penalty_ammount, borderCharges, remarkOnDriver, booking } =
        matchingInvoice;
  ...
  const filterOnlyBooking = () => {
    const filteredData = bookingsData.filter(
      (item: any) =>
        item.booking_status === "pending" && item.invoice_status != "generated"
    );
  ...
  const filterOnlyInvoice = () => {
    const filteredData = bookingsData.filter(
      (item: any) => item.invoice_status === "generated"
    );
    setFilteredBookingsData(filteredData);
  };
  ...
  const findBookingAmounts = (bookingId: any, type: string) => {
    const selectedBooking = bookingsData.find(
      (id: any) => id?.new_booking_id === bookingId
    );
    ...
    for (const id in driver) {
      const waitingAmt = parseInt(driver[id].Waiting_Amount)
        ...
        : 0;
        ///console.log('pay-----',driver[id]);
        
      const paidAmount = driver[id].pay_amount ? driver[id].pay_amount : 0;
      const total =
    ...
    // }
    console.log(totalAmount, paidAmount, "000000000000000000000");
  };

  const viewDriver = (bookingId: any) => {
    setViewDriverMode(true);
    const selectedBooking = bookingsData.find(
      (id: any) => id?.new_booking_id === bookingId
    );
    console.log(selectedBooking, "selectedBooking");
    const SelectedDriver = JSON.parse(selectedBooking?.drivers);
    const FindDriver = [];
    const filterDriver = [];
    ...
    // for (const id in driver) {
    //   const currentDriver = driver[id].amount
    //   const currentDriverData = driverData.find((name: any) => name.driver_id === parseInt(id));

    //   if (currentDriverData) {
    //     filterDriver.push({
    //       id: parseInt(id),
    //       name: currentDriverData.name,
    //       Driver_Payable: parseInt(currentDriver.amount),
    //       Border_Charges: selectedBooking.all_border_fare,
    //       Waiting_Amount: parseInt(currentDriver.Waiting_Amount) || 0,
    //       total: parseInt(currentDriver.amount) + selectedBooking.all_border_fare + (parseInt(currentDriver.Waiting_Amount) || 0),
    //     });
    //   }
    // }
  ...
  return (
    <div>
        ...
        <div className="flex sm:flex-row flex-col sm:items-center sm:gap-3 gap-4 w-full sm:w-auto">
          <div className="flex gap-3">
            ...
            <div>
              <ul>
                <li>
                  <NavLink className="btn btn-primary" to="/booking/NewBooking">
                    <IconUserPlus className="ltr:mr-2 rtl:ml-2" /> Add New
                    Booking
                  </NavLink>
                </li>
              </ul>
                ...
                {filteredBookingsData?.map((contact: any, index: any) => {
                  ...
                  return (
                    <tr key={contact.id}>
                      ...
                      <td className="whitespace-nowrap">
                        {contact.new_booking_id}
                      </td>
                      <td className="whitespace-nowrap">
                        {moment(contact?.date).format("MM/DD/YYYY")}
                      </td>
                      <td className="whitespace-nowrap">
                        {contact?.border_Route.route_name}
                      </td>
                      <td className="whitespace-nowrap">
                      ...
                      <td className="whitespace-nowrap">
                        {findBookingAmounts(contact.new_booking_id, "balance")}
                      </td>
                      <td className="whitespace-nowrap">
                        {contact.trackingsses &&
                          contact?.trackingsses.length > 0 ? (
                          // Use lodash's get function to safely navigate nested properties
                          _.get(
                            contact.trackingsses[0],
                        ...
                        {contact?.document_path == "" ||
                          ...
                          <div className="flex gap-4 items-center justify-center">
                            <button
                              type="button"
                              className="btn btn-sm btn-outline-primary"
                              onClick={() => handleDownloadDocument(contact)}
                            >
                              View Document
                            </button>
                          </div>
                        ...
                        {contact.invoice_status !== "generated" ? (
                          <div className="flex gap-4 items-center justify-center">
                            <button
                              ...
                              onClick={() => {
                                navigate(
                                  `/booking/invoice/${contact?.booking_id}`
                                );
                                // editUser(contact, "invoice");
                              }}
                      ...
                      <td>
                        <div className="flex gap-4 items-center justify-center">
                          <button
                            ...
                            className="btn btn-sm btn-outline-primary"
                            onClick={() => {
                              navigate(`/booking/view/${contact?.booking_id}`);
                              // ViewUser(contact);
                            }}
                          >
                            view
                          ...
                          <button
                            ...
                            className="btn btn-sm btn-outline-primary"
                            onClick={() => {
                              navigate(`/booking/edit/${contact?.booking_id}`);
                              // editUser(contact, "edit");;
                            }}
                          >
                            Edit
                          </button>
                          <button
                            ...
                            disabled={
                              contact?.invoice_status === "generated" ||
                              contact?.booking_status === "complete"
                            }
                          >
                            {contact.booking_status === "pending"
                              ? "Cancel"
                              : "Booking Canceled"}
                          </button>
                        </div>
                      ...
                      <div className="grid mt-4 grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                          ...
                          <input
                            ...
                            value={
                              params.new_booking_id || generateBookingNumber()
                            }
                            type="text"
                            placeholder="Enter Booking Id"
                            className="form-input"
                            disabled={
                              viewMode || params.booking_id || viewInvoiceType
                            }
                            required
                          />
                        </div>
                      ...
                      <div className="mt-4">
                        <div>
                          ...
                          <select
                            ...
                            disabled={
                              viewMode || viewInvoiceType || addInvoiceType
                            }
                            required
                          >
                                ...
                                value.customer_id ===
                                  ...
                                  <div className="flex">
                                    ...
                                    <label className="ml-8 mt-2 font-bold text-red-500">
                                      Credit Use:{" "}
                                      {value.credit_limit - value.balance}
                                    </label>
                                  </div>
                                ) : (
                                  ""
                                    ...
                                    <div className="overflow-x-auto">
                                      <table className="min-w-full table-auto">
                                        ...
                                        <tbody>
                                          <tr>
                                            <td
                                              className={`pr-4 font-bold ${new Date(d.truckExpiryDate) <
                                                  new Date()
                                                  ? "text-red-500"
                                                  : "text-green-500"
                                                }`}
                                        ...
                                        <tbody>
                                          <tr>
                                            <td
                                              className={`pr-4 font-bold ${new Date(
                                        ...
                                        <tbody>
                                          <tr>
                                            <td
                                              className={`pr-4 font-bold ${new Date(d.passportExpiryDate) <
                                                  new Date()
                                                  ? "text-red-500"
                                                  : "text-green-500"
                                                }`}
                                        ...
                                        <tbody>
                                          <tr>
                                            <td
                                              className={`pr-4 font-bold ${new Date(d.idCardExpiryDate) <
                                                  new Date()
                                                  ? "text-red-500"
                                                  : "text-green-500"
                                                }`}
                        ...
                        <div>
                          ...
                          <select
                            ...
                            onChange={(e) => {
                              ...
                              const selectedRouteData = routeData.find(
                                (route: any) =>
                                  route.route_id === parseInt(params.route_id)
                              );
                              setFilterrouteData(selectedRouteData);
                              console.log(
                                selectedRouteData,
                          ...
                          >
                            ...
                            {routeData.map((country: any) => (
                              <option
                                key={country?.route_id}
                                value={country?.route_id}
                              >
                                {country?.route_name}
                              </option>
                            ))}
                          ...
                          {!viewInvoiceType ? (
                            ...
                            <div className="mt-4">
                              ...
                              <button
                                ...
                                className="btn btn-sm btn-outline-primary"
                                onClick={() =>
                                  handleDownloadConsignmentReceipt()
                                }
                              >
      ...
      <Transition appear show={viewDriverMode} as={Fragment}>
        <Dialog
          as="div"
          open={viewDriverMode}
          onClose={() => {
            setViewDriverMode(false);
          }}
          className="relative z-[51]"
        >
          <Transition.Child
